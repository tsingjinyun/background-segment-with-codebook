...
void cvupdateCodeBook(uchar *p, codeBook &c, unsigned *cbBounds, int numChannels)
{
	...
	// ENTER A NEW CODE WORD IF NEEDED：添加一个新的codebook
	...
	// OVERHEAD TO TRACK POTENTIAL STALE ENTRIES:统计每个码本条目多长时间被访问一次
	...
	// SLOWLY ADJUST LEARNING BOUNDS：慢慢调整学习上下界限
	for(n=0; n<numChannels; n++)
...
//函数功能：使用经验模型在先前的背景中将前景目标的像素分割出来
...
int cvclearStaleEntries(codeBook &c)  //如果codebook的条目i在总运行时间一半的时间段内没有被访问，该条目（keep[i]=0）被删除
{
	int staleThresh = c.t >> 1;			// 设定刷新时间：总运行时间的一半
